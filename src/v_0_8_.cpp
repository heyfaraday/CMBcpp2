...
int main() {
    ...
    for (unsigned int i = 0; i < npix; ++i) {
        for (unsigned int j = 1; j < npix / 2; ++j) {
            ...
            p_x[i][j] = (2.0L * q[i][j] * q_x[i][j] + 2.0L * u[i][j] * u_x[i][j]) / (2.0L * p[i][j]);
            p_y[i][j] = (2.0L * q[i][j] * q_y[i][j] + 2.0L * u[i][j] * u_y[i][j]) / (2.0L * p[i][j]);
        }
    }
    ...
    singular_points_classifier(q, u, cos_ml_q, sin_ml_q, cos_ml_u, sin_ml_u, "v_0_8_1_sing_points.dat", whitelist);


    std::cout << "6" << std::endl;

    points_classifier(p_x, p_y, cos_ml, sin_ml, "v_0_8_1_points.dat", whitelist);
