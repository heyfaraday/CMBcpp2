...
long double s2(long double phi1, long double phi2, long double theta1, long double theta2) {
    ...
    return acosl(sinl(theta1) * sinl(theta2) + cosl(theta1) * cosl(theta2) * cosl(phi1 - phi2));
}
...
void cross(long double* phi_out, long double* theta_out,
    ...
    long double a1[3] = {cosl(theta1a) * cosl(phi1a), cosl(theta1a) * sinl(phi1a), sinl(theta1a)};
    long double b1[3] = {cosl(theta1b) * cosl(phi1b), cosl(theta1b) * sinl(phi1b), sinl(theta1b)};
    long double c1[3] = {a1[1] * b1[2] - a1[2] * b1[1], a1[2] * b1[0] - a1[0] * b1[2], a1[0] * b1[1] - a1[1] * b1[0]};

    long double a2[3] = {cosl(theta2a) * cosl(phi2a), cosl(theta2a) * sinl(phi2a), sinl(theta2a)};
    long double b2[3] = {cosl(theta2b) * cosl(phi2b), cosl(theta2b) * sinl(phi2b), sinl(theta2b)};
